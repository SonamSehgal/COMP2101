...
test -h /etc/resolv.conf && echo "/etc/resolv.conf is a symbolic link" || echo "/etc/resolv.conf is not a sysmbolic link."
...
test -w /etc/resolv.conf && echo "/etc/resolv.conf is a writable" || echo "/etc/resolv.conf is not writable."
...
test -r /tmp && echo "/tmp is readable" || echo "/tmp is readable."


# TASK 5: Add a test to see if the /tmp directory is writable
test -w /tmp && echo "/tmp is writable" || echo "/tmp is writable."
...
read firstNumber1 secondNumber2
...
if [ `expr $secondNumber % $firstNumber` == 0 ]
then 
	echo "$secondNumber is a multiple of $firstNumber"
else
	echo "$secondNumber is not a multiple of $firstNumber"
...
[ -s USER ] && echo " The content in USER is: $USER" || echo "The variable is empty"

#TASK 2: Add a command that tells the user if the USER variable exists, but is empty
[ -v USER ] && echo "The variable SHELL exists"
[ -z USER ] && echo " The content is not empty" || echo "The variable is empty"
...
read -p "Enter a string" $stra
read -p "Enter another string" $strb
! [ $stra != $strb ] && echo " $stra is not alphanumerically equal to $strb" || echo "$stra is alphanumerically equal to $strb"
